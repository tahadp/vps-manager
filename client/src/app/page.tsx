...
import { 
  ...
  Power, PowerOff, RefreshCw, TerminalSquare, AlertCircle, Play
} from "lucide-react";
          ...
          {vpsList.map(vps => {
            ...
            return (
              <motion.div 
                ...
                key={vps.id} 
                className={`group relative bg-neutral-bg2/80 backdrop-blur-xl border ${isSelected ? 'border-brand shadow-glow' : 'border-border-DEFAULT hover:border-brand/50'} rounded-2xl overflow-hidden transition-all duration-300 flex flex-col`}
              >
                {/* Checkbox */}
                ...
                {/* Header */}
                <div className="p-5 pb-4 relative z-10 cursor-pointer" onClick={() => router.push(`/vps/${vps.id}`)}>
