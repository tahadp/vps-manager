...
import { ArrowLeft, Bell, Plus, Trash2, Filter, ChevronDown, Server, AlertCircle, Edit3, X, Save, Shield, Clock, Cpu, MemoryStick, HardDrive } from 'lucide-react';
...
export default function VpsAlertsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [vpsName, setVpsName] = useState<string>('');
  const [loading, setLoading] = useState(true);

  // Filters
  const [filterType, setFilterType] = useState<string>('all'); // all, specific, global
  ...
  useEffect(() => {
    ...
    // Fetch VPS details and rules
    Promise.all([
      api<any>(`/api/vps/${id}`),
      api<any[]>('/api/rules')
    ])
      .then(([vpsData, rulesData]) => {
        if (vpsData && vpsData.name) {
          setVpsName(vpsData.name);
        }
        if (Array.isArray(rulesData)) {
          setRules(rulesData.filter((r: any) => !r.vpsId || r.vpsId === id));
        }
      })
      .catch(() => {})
    ...
    const onEvent = (e: any) => {
      if (e?.type === 'RULES_CHANGED') {
        api<any[]>('/api/rules')
          .then(d => {
            if (Array.isArray(d)) {
              setRules(d.filter((r: any) => !r.vpsId || r.vpsId === id));
            }
          })
          .catch(() => {});
      }
  ...
  const filteredRules = rules.filter(r => {
    // Type Filter (All, specific to this VPS, global rules)
    if (filterType === 'specific' && r.vpsId !== id) return false;
    if (filterType === 'global' && r.vpsId !== null) return false;
  ...
  const saveRule = async () => {
    try {
      ...
      if (!payload.vpsId) payload.vpsId = null;

      if (editingRuleId) {
        // Update existing rule
        const r = await api<any>(`/api/rules/${editingRuleId}`, { method: 'PUT', json: payload });
        setRules(rules.map(rule => rule.id === editingRuleId ? r : rule));
      } else {
        // Create new rule
      ...
      setNewRule({ vpsId: id, metric: 'CPU', condition: '>', threshold: 90, durationMin: 10, offlineThresholdMin: 5, customMessage: '', restartOnAlert: false, action: 'ALERT', script: '' });
    } catch (err: any) {
  ...
  const startEdit = (rule: any) => {
    ...
    setNewRule({
      ...
      restartOnAlert: rule.restartOnAlert || false,
      action: rule.action,
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12 px-4">
      <header className="mb-6 flex items-center gap-4">
        <button onClick={() => router.push(`/vps/${id}`)} className="w-10 h-10 rounded-xl bg-neutral-bg2 border border-border-DEFAULT flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-neutral-bg3 transition-colors shadow-sm">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-text-primary flex items-center gap-3">
            <Bell className="w-7 h-7 text-brand" /> {vpsName ? `${vpsName} - ` : ''}Alert Rules
          </h1>
          <p className="text-text-secondary text-sm">Configure alert rules specific to this VPS or toggle global rule targets.</p>
        </div>
      </header>
      ...
      <div className="bg-neutral-bg2/40 border border-border-subtle rounded-2xl p-4 mb-4 flex flex-wrap items-center gap-3 backdrop-blur-md">
        ...
        <select value={filterType} onChange={e => setFilterType(e.target.value)} className="px-3 py-1.5 text-sm bg-neutral-bg1 border border-border-subtle rounded-lg text-text-primary focus:outline-none focus:border-brand">
          <option value="all">All Applicable Rules</option>
          <option value="specific">VPS Specific Only</option>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-neutral-bg2/80 border border-border-DEFAULT rounded-2xl p-6 mb-6 backdrop-blur-xl shadow-xl space-y-4 overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-text-muted mb-1.5 uppercase">Target VPS Scope</label>
                <select value={newRule.vpsId} onChange={e => setNewRule({ ...newRule, vpsId: e.target.value })} className="w-full p-2.5 rounded-xl bg-neutral-bg1 border border-border-DEFAULT text-text-primary text-sm focus:outline-none focus:border-brand">
                  <option value={id}>This VPS Only ({vpsName})</option>
                  <option value="">All VPS (Global Rule)</option>
                </select>
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredRules.length === 0 ? (
          <div className="col-span-full bg-neutral-bg2/40 border border-dashed border-border-strong rounded-2xl p-12 text-center text-text-muted">
            ...
            <p className="text-xs text-text-muted mt-1">Try tweaking your filters or add a new rule.</p>
          </div>
        ) : (
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-xs font-semibold text-text-muted uppercase tracking-wider">Target Scope</div>
                      <div className="text-sm font-bold text-text-primary flex items-center gap-1.5">
                        ...
                        {rule.vpsId ? <span className="text-text-primary">This VPS Only</span> : <span className="text-brand-light font-medium">All VPS (Global)</span>}
                      </div>
                    </div>
                  </div>
